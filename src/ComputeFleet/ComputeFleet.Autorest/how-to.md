# How-To
This document describes how to develop for `Fleet`.
...
There are multiple scripts created for performing different actions for developing `Fleet`.
- `build-module.ps1`
  - Builds the module DLL (`./bin/Fleet.private.dll`), creates the exported cmdlets and documentation, generates custom cmdlet test stubs and exported cmdlet example stubs, and updates `./Fleet.psd1` with Azure profile information.
  - **Parameters**: [`Switch` parameters]
    - `-Run`: After building, creates an isolated PowerShell session and loads `Fleet`.
...
- `run-module.ps1`
  - Creates an isolated PowerShell session and loads `Fleet` into the session.
